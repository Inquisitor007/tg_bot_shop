...
async def order_invoice(callback: CallbackQuery, widget: Button, dialog_manager: DialogManager, **kwargs):
    ...
    user_cart: Cart = await sync_to_async(lambda: user.cart)()
    ...
    order = await create_order(dialog_manager, user, user_cart, cart_items)
...
async def create_order(dialog_manager: DialogManager, user: User, user_cart: Cart, items: list[CartItem]):
    dialog_data = dialog_manager.current_context().widget_data
    address = dialog_data.get('address')
    fio = dialog_data.get('fio')
    total_price = await sync_to_async(user_cart.total_price)()
    order = await sync_to_async(Order.objects.create)(user=user, address=address, fio=fio, total_price=total_price)
